...
class Login extends Component {
    ...
    handleSearch() {
        ...
        if (search.length === 0) {
            search = "land"
            this.setState({items: [], docs: [], pagenum: 1})
        }
        ...
        try {
            let search = this.state.search
            trackPromise(call(`https://api.nytimes.com/svc/search/v2/articlesearch.json?q=` + search + `&api-key=LQUux1EMbIoJJOSuYcdej2CxEqTz8QEx&page=` + pagenum + ``).then(response => console.log(response.response.docs.map(doc => {
                this.setState({
                    items: [
                        ...this.state.items,
                        doc
                ...
                <div className="header">
                    <div className="headercontent">
                        <div className="searchbar">
                            ...
                            <div className="account">
                                <button>
                                    <Link to="/signin" onClick={this.handleLogout}>Logout</Link>
                                </button>
                            </div>
                            <div className="account">
                                <button>
                                    <Link to="/follow">Add Followers</Link>
                                </button>
                            </div>
                        </div>
                    </div>
